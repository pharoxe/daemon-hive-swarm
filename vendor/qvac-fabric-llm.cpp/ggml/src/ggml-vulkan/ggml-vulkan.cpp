...
static vk_device ggml_vk_get_device(size_t idx) {
    ...
    if (vk_instance.devices[idx] == nullptr) {
        ...
        if (descriptor_buffer_support && getenv("GGML_VK_TILING_DISABLE") == nullptr &&
            (device->architecture == vk_device_architecture::QUALCOMM_ADRENO ||
             device->architecture == vk_device_architecture::ARM_MALI)) {
            // Despite being able to allocate large buffers, using them for SSBOs causes problems
            // on Adreno and Mali GPUs — tile larger matmuls into smaller submissions.
            uint64_t threshold = descriptor_buffer_props.descriptorBufferAddressSpaceSize;
            if (device->architecture == vk_device_architecture::ARM_MALI) {
                constexpr uint64_t kMaliMaxTileBytes = 64u * 1024u * 1024u;
                threshold = std::min(threshold, kMaliMaxTileBytes);
            }
            device->tiling_threshold = threshold;
        }
...
static uint32_t ggml_vk_guess_split_k(ggml_backend_vk_context * ctx, uint32_t m, uint32_t n, uint32_t k, bool disable_split_k, const vk_pipeline& pipeline) {
    ...
    // disable split_k for large Adreno/Mali matrices — tiling handles those cases and
    // split_k would increase memory requirements on embedded GPU drivers.
    if (ctx->device->architecture == vk_device_architecture::QUALCOMM_ADRENO ||
        ctx->device->architecture == vk_device_architecture::ARM_MALI) {
        return 1;
    }
